...
class ConfigController extends Controller
{
    ...
    public function configuration(): JsonResponse
    {
        ...
        $payconiq_payment = json_decode($this->business_setting->where(['key' => 'payconiq_payment'])->first()->value, true);
        $bancontact = $payconiq_payment ? $payconiq_payment['status'] : 0;
	    $paypal_payment = json_decode($this->business_setting->where(['key' => 'paypal'])->first()->value, true);
        $paypal = $paypal_payment ? $paypal_payment['status'] : 0;
		$wolt_delivery = json_decode($this->business_setting->where(['key' => 'wolt_service'])->first()->value, true);
        $wolt = $wolt_delivery ? $wolt_delivery['status'] : 0;
		$paystack_payment = json_decode($this->business_setting->where(['key' => 'paystack'])->first()->value, true);
        $paystack = $paystack_payment ? $paystack_payment['status'] : 0;
        $stripe_payment = json_decode($this->business_setting->where(['key' => 'stripe'])->first()->value, true);
		$language = json_decode($this->business_setting->where(['key' => 'language'])->first()->value, true);
        $dineInn = json_decode($this->business_setting->where(['key' => 'dine_in'])->first()->value, true);
        $takeWay = json_decode($this->business_setting->where(['key' => 'take_way'])->first()->value, true);
        $delivery = json_decode($this->business_setting->where(['key' => 'deliver'])->first()->value, true);
        ...
        $map_key = $this->business_setting->where(['key' => 'map_api_server_key'])->first()->value ?? null;
        ...
        return response()->json([
            ...
            'theme' => (int) $this->business_setting->where(['key' => 'theme'])->first()->value,
