...
class TableController extends Controller
{
	...
	public function getTableGroup(){
		...
		$groups = $this->group->with('table')->where('is_available', 1)
			  ->where('status', 1)->get();
	...
	public function add(Request $request)
	{
		...
		// Handle branch (assuming $branch can be an object or ID)
		$branch = $request->branch;

		// Assign branch_id to validated data
		if (is_object($branch) && isset($branch->id)) {
			$validatedData['branch_id'] = $branch->id;
		} elseif (is_numeric($branch)) {
			$validatedData['branch_id'] = $branch;
		} else {
			return response()->json(['error' => 'Invalid branch data'], 400);
		}
	...
	public function update(Request $request)
	{
		...
		$validatedData = $request->validate([
			'id' => 'required|exists:tables,id', // Ensure the ID exists in the tables table
			'group_id' => 'required|integer',
			'number' => 'required|string',
			'capacity' => 'required|integer',
		]);

		// Find the table by ID or fail
		$table = Table::findOrFail($request->id);

		
		$table->group_id = $request->group_id ?? $table->group_id;
		$table->number = $request->number ?? $table->number;
	...
	{
		// Validate the incoming request to ensure an ID is provided
		$validatedData = $request->validate([
			'id' => 'required|exists:tables,id', // Ensure the ID exists in the `tables` table
		]);

		// Find the table by ID or fail
		$table = Table::findOrFail($validatedData['id']);
