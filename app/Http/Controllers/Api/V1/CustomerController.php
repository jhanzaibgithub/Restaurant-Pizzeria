...
class CustomerController extends Controller
{
    ...
    public function update_profile(Request $request): JsonResponse
    {
        ...
        if ($request->has('image')) {
            $userDetails['image'] = Helpers::update('profile/', $request->user()->image, 'png', $request->file('image'));
            
        } else {
            $userDetails['password'] = (isset($request->password) && strlen($request['password']) > 5) ? bcrypt($request['password']) : $request->user()->password;
    
            $userDetails['f_name'] = isset($request->f_name) ? $request->f_name : $request->user()->f_name;
    
            $userDetails['l_name'] = isset($request->l_name) ? $request->l_name : $request->user()->l_name;
    
            $userDetails['phone'] = isset($request->phone) ? $request->phone : $request->user()->phone;
        }
