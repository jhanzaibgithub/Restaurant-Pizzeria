...
class ProductController extends Controller
{
    ...
    public function index(): Renderable
    {
        ...
        return view('admin-views.product.index', compact('categories', 'addons', 'languageSettings'));
    }
    ...
    public function list(Request $request): Renderable
    {
        ...
        if ($request->has('search')) {
            ...
            if (session()->has('product_filter')) {
                $product_filter = json_decode(session('product_filter'));

                $query = Product::with(['branch_products'])
                    ->when(isset($product_filter->product_type) && $product_filter->product_type != 'all', function ($query) use ($product_filter) {
                        return $query->where('product_type', $product_filter->product_type);
                    })
                    ->when(isset($product_filter->branch_id), function ($query) use ($product_filter) {
                        return $query->whereHas('branch_products', function ($q) use ($product_filter) {
                            return $q->where('branch_id', $product_filter->branch_id);
                        });
                    })
                    ->when(isset($product_filter->product_section), function ($query) use ($product_filter) {
                        return $query->whereJsonContains('category_ids', ['id' => $product_filter->product_section]);
                    })
                    ->when(isset($product_filter->status), function ($query) use ($product_filter) {
                        return $query->where('status', $product_filter->status);
                    })
                    ->when(isset($product_filter->priority) && $product_filter->priority != 'all', function ($query) use ($product_filter) {
                        return $query->where('priority', $product_filter->priority);
                    });
            }
        }

        $products = $query->orderByRaw('CASE WHEN priority = 2 THEN 0 ELSE 1 END')
        ...
        $product_type = isset($product_filter->product_type) ? $product_filter->product_type : null;
        $branch_id = isset($product_filter->branch_id) ? $product_filter->branch_id : null;
        $product_section = isset($product_filter->product_section) ? $product_filter->product_section : null;
        $status = isset($product_filter->status) ? $product_filter->status : null;
        $priority = isset($product_filter->priority) ? $product_filter->priority : null;
    ...
    {
        session()->put('product_filter', json_encode($request->all()));
        return back();
    }
    ...
    {
        $product = $this->product->withCount('reviews')->where(['id' => $id])->first();
        $productAddOns = AddOn::whereIn('id', json_decode($product['add_ons'], true) ?: [])->get();
        $reviews = Review::where(['product_id' => $id])->latest()->paginate(20);
        return view('admin-views.product.view', compact('product', 'reviews', 'productAddOns'));
    }
    ...
    public function edit($id): Renderable
    {
        ...
        return view('admin-views.product.edit', compact('product', 'product_category', 'categories', 'addons', 'languageSettings'));
    }
    ...
    public function update_priority(Request $request, $id)
    {
        ...
        Toastr::success(translate('Product priority updated!'));
        return back();
    }
    ...
    public function update(Request $request, $id): JsonResponse
    {
        ...
        $product->set_menu = $request->item_type;
        $product->product_type = $request->product_type;
        $product->image = Helpers::update('product/', $product->image, 'png', $request->file('image'));
        $product->available_time_starts = $request->available_time_starts;
        $product->available_time_ends = $request->available_time_ends;
