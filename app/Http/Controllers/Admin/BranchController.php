...
class BranchController extends Controller
{
    ...
    public function index(): Renderable
    {
        ...
        return view('admin-views.branch.index', compact('mapApiClientKey'));
    }
    ...
    public function edit($id): Renderable
    {
        ...
        return view('admin-views.branch.edit', compact('branch', 'mapApiClientKey'));
    }
    ...
    public function list(Request $request): Renderable
    {
        ...
        if($request->has('from') && $request->has('to')){
            $query = $this->branch->whereBetween('created_at', [$request->from, $request->to]);
        }  
        ...
        foreach ($branches as $branch) {
            $currentMonthOrders = $this->order->where('branch_id',$branch->id)
                ->whereBetween('created_at', [now()->startOfMonth(), now()])
                ->count();
            $previousMonthOrders = $this->order->where('branch_id',$branch->id)
                ->whereBetween('created_at', [now()->subMonth()->startOfMonth(), now()->subMonth()->endOfMonth()])
                ->count();
            $growthPercentage = 0;
            if ($previousMonthOrders > 0) {
                $growthPercentage = ($currentMonthOrders - $previousMonthOrders) / $previousMonthOrders * 100;
            }
            $branch->trend = $currentMonthOrders >= $previousMonthOrders ? 'up' : 'down';
            $branch->growth = $growthPercentage;
        }
