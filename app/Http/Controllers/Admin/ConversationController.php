...
class ConversationController extends Controller
{
    ...
    public function list(): Renderable
    {
        ...
        $conversationSidebar = $this->buildConversationSidebarData($conversations);
        $firebaseConfig = Helpers::get_business_settings('firebase_message_config');

        return view('admin-views.messages.index', compact('conversations', 'conversationSidebar', 'firebaseConfig'));
    }
    ...
    public function get_conversations(): JsonResponse
    {
        ...
        return response()->json([
            'conversation_sidebar' => view('admin-views.messages.partials._list', compact('conversationSidebar'))->render(),
        ]);
    }
    ...
    private function buildConversationSidebarData($conversations): array
    {
        $userIds = $conversations->pluck('user_id')->filter()->unique()->values();
        $users = $this->user->whereIn('id', $userIds)->get()->keyBy('id');
        $missingUserIds = $userIds->diff($users->keys());

        if ($missingUserIds->isNotEmpty()) {
            $this->conversation->whereIn('user_id', $missingUserIds)->update(['checked' => 1]);
        }

        $uncheckedCounts = $this->conversation
        ...
        return $userIds
            ...
            ->map(fn($userId) => [
                'user' => $users[$userId],
                'unchecked' => (int) ($uncheckedCounts[$userId] ?? 0),
            ])
            ->values()
            ->all();
    }
