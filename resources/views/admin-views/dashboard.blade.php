                ...
                <ul class="list-unstyled">
                    <div>
                        <li class="{{ Request::is('admin/dashboard') ? 'active' : '' }}"><a
                                href="{{ route('admin.dashboard') }}">{{ translate('Overview') }}</a></li>
                    </div>
                </ul>
                    ...
                    <div class="lower-section card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-xs-4 mb-3">
                                <div class="card">
                                    <div style="padding: 0.3125rem;" class="card-body ml-3">
                                        @if ($analytics['employee_trend'] == 'up')
                                            <div class="mb-3 font-size-lg text-success">
                                                <strong><i class="tio-trending-up  display-4"></i></strong>
                                            </div>
                                        @else
...
@push('script_2')
    ...
    <script>
                ...
                <div class="d-flex flex-column gap-3">
                    @foreach ($data['top_sell_products'] as $key => $item)
                        @if (isset($item->product))
                            <a  class="d-flex justify-content-around align-items-center text-dark" href='{{ route('admin.product.view', [$item['product_id']]) }}'">
        ...
        function showDataDelivery(category) {
            ...
            <div class="card-body">
                <div class="d-flex flex-column gap-3">
                    @foreach ($data['branches'] as $key => $item)

                            <a  class="d-flex justify-content-around align-items-center text-dark" href='{{ route('admin.product.list') }}'">
                                <div style="width:50%" class="media align-items-center gap-2  flex-grow-1">
                                    <img class="rounded avatar avatar-lg" src="{{ asset('/storage/branch') . '/' . $item['branch']['cover_image'] ?? '' }}" onerror="this.src='{{ asset('assets/admin/img/400x400/img2.jpg') }}'" alt="$item['branch']['name'] image">
                                    <div class="d-flex flex-column">
                                    <span class="font-weight-semibold text-capitalize media-body">{{ substr($item['branch']['name'], 0, 18) }}</span>
                                    <span style="color:#A1A5B7;" class="font-weight-semibold text-capitalize media-body">#{{ $item['branch']['id'] }}</span>
                                    </div>
                                </div>
                                <div class="media align-items-center gap-2  flex-grow-1">
                                    <div class="d-flex flex-column">
                                    <span class="font-weight-semibold text-capitalize media-body">{{ $item['total_delivered_orders'] }}</span>
                                    <span style="color:#A1A5B7;" class="font-weight-semibold text-capitalize media-body">Orders</span>
                                    </div>
                                </div>
                                <div class="media align-items-center gap-2  flex-grow-1">
                                    <div class="d-flex flex-column">
                                    <span class="font-weight-semibold text-capitalize media-body">{{ $item['total_product_count'] }}</span>
                                    <span style="color:#A1A5B7;" class="font-weight-semibold text-capitalize media-body">Products</span>
                                    </div>
                                </div>
                                <div class="media align-items-center gap-2  flex-grow-1">
                                    <div class="d-flex flex-column">
            ...
            $('#homeDeliveryCard').find('img').attr('src',
                '{{ asset('assets/admin/img/dashboard/dashboard_branch.png') }}');
        }
        ...
        function resetCardStyles() {
            ...
            $('#homeDeliveryCard img').attr('src', '{{ asset('assets/admin/img/dashboard/dashboard_branch_grey.png') }}');
        }
            ...
            updateElement: function(arc, index, reset) {
                ...
                helpers.extend(arc, {
                    ...
                    draw: function() {
                        ...
                        ctx.fillText('{{ $totalReviews }}', vm.x, vm.y - 40, 400);
            ...
            function generateHeatmapData(orders_json) {
                ...
                orders_json.forEach(function(order) {
                    ...
                    if (orderCounts[location] === undefined) {
                        orderCounts[location] = 1;
                    } else {
                        orderCounts[location]++;
                    }
                });
        ...
        var earningOptions = {
            ...
            series: [{
                ...
                data: [{{ $earning[1] }}, {{ $earning[2] }}, {{ $earning[3] }}, {{ $earning[4] }},
                    {{ $earning[5] }}, {{ $earning[6] }},
                    {{ $earning[7] }}, {{ $earning[8] }}, {{ $earning[9] }},
                    {{ $earning[10] }}, {{ $earning[11] }}, {{ $earning[12] }}
                ],
            }, ],
            markers: {
                size: 2,
    ...
    <script>
        ...
        function saleGrowthStatisticsUpdate(t) {
            ...
            $.ajax({
                url: '{{ route('admin.sale-statistics') }}',
                type: 'GET',
                data: {
                    type: value
                },
