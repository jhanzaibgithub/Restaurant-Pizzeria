...
@include('admin-views.customer.partials._customers-setup-inline-menu')
    ...
    <div class="content container-fluid">
                    ...
                    <div class="col-lg-12 pt-3">
                        <form action="{{route('admin.customer.loyalty-point.report')}}" method="get">
                            ...
                            <div class="btn--container justify-content-end">
                                ...
                                <button type="reset" id="reset_btn" class="btn btn-white text-order_id border-primary">{{translate('reset')}}</button>
        ...
        <div class="card mt-3">
            <div class="card-top px-card py-3">
                    ...
                    <table id="datatable"
                        ...
                        @foreach($transactions as $k=>$wt)
                            <tr scope="row">
                                ...
                                <td><a a class="text-muted" href="{{route('admin.customer.view',['user_id'=>$wt->user_id])}}">{{Str::limit($wt->customer?$wt->customer->f_name.' '.$wt->customer->l_name:translate('not_found'),20,'...')}}</a></td>
                                <td>{{$wt->credit}}</td>
                                <td>{{$wt->debit}}</td>
                                <td>{{$wt->amount}}</td>
                                <td>
                                    <span style="text-decoration:underline" class=" text-{{$wt->type=='point_to_wallet'?'success':'dark'}}">
                                        {{translate($wt->type)}}
                                    </span>
                                </td>
