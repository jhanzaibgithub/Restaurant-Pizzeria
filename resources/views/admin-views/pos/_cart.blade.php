        ...
        <table class="table table-align-middle mb-0">
            ...
            @if(session()->has('cart') && count( session()->get('cart')) > 0)
                ...
                @if(is_array($cartItem))
                    ...
                    //tax calculation
                    $product = $cartProducts[$cartItem['id']] ?? null;
                    $total_tax += $product ? \App\CentralLogics\Helpers::tax_calculate($product, $cartItem['price'])*$cartItem['quantity'] : 0;
                    ...
                    <tr>
                                    ...
                                    <small class="d-block">
                                        ...
                                        @foreach($cartItem['add_ons'] as $key2 =>$id)
                                           ...
                                           <div class="font-size-sm text-body">
                                               <span> <small class="font-weight-bold">+{{ $addon['name'] ?? translate('addon deleted') }}</small></span>
                                                <span><small class="font-weight-bold text-warning">
                                                    <!-- {{ $add_on_qty}} x {{ \App\CentralLogics\Helpers::set_symbol($addon['price']) }} -->
                                                    {{ \App\CentralLogics\Helpers::set_symbol($addon['price'] ?? 0) }}
                                                </small></span>
                                            </div>
                                           </div>
                                        @endforeach
                        ...
                        <td>
                           ...
                           <div class="d-flex">
                                <button class="quantity-button btn-warning" onclick="decreaseQuantity(event)">-</button>
                                <input type="number" style="border:none; width: 40px; height: 30px;" class="form-control qty" data-key="{{$key}}" value="{{$cartItem['quantity']}}" min="1" >
                                <button class="quantity-button btn-warning" onclick="increaseQuantity(event)">+</button>
                           </div>
                        </td>
                        <td>
                            <div class="">
    ...
    <div class="pos-data-table p-3">
        <dl class="row">
            ...
            <dt  class="col-6 border-top pt-2">{{translate('VAT/TAX:')}} : </dt>
            <dd class="col-6 border-top text-right">{{ \App\CentralLogics\Helpers::set_symbol(round($total_tax + $addon_total_tax + $overall_tax,2)) }}</dd>

            <dt  class="col-6 border-top font-weight-bold pt-2">{{translate('total')}} : </dt>
            <dd class="col-6 text-right border-top font-weight-bold pt-2">{{ \App\CentralLogics\Helpers::set_symbol(round($total+$total_tax+$addon_total_tax + $overall_tax, 2)) }}</dd>
        </dl>



        <form action="{{route('admin.pos.order')}}" method="post" id='order_place' >
            @csrf
