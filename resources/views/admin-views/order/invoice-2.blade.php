...
@php($businessSettings = $businessSettings ?? [])
@php($addonMap = $addonMap ?? collect())
...
<main id="content" role="main" class="main pointer-event">
    <div class="content container-fluid">
        ...
        <div class="row">
            <div class="col-4">
                <img width="150"
                     src="{{asset('/storage/restaurant')}}/{{$businessSettings['logo'] ?? ''}}">
                <br><br>
                <strong>Phone : {{$businessSettings['phone'] ?? ''}}</strong><br>
                <strong>Email : {{$businessSettings['email_address'] ?? ''}}</strong><br>
                <strong>Address : {{$businessSettings['address'] ?? ''}}</strong><br><br>
            </div>
            <div class="col-4"></div>
            <div class="col-4">
                @if($order->customer)
                        ...
                        <div class="row">
                            <div class="col-12 pb-2 border-bottom">
                                <h4 class="card-header-title">
                                    Order details
                                    <span
                                        class="badge badge-soft-dark rounded-circle ml-1">{{count($order->details ?? [])}}</span>
                                </h4>
                            </div>
                                                ...
                                                @foreach(json_decode($detail['add_on_ids'],true) as $key2 =>$id)
                                                    ...
                                                    <div class="font-size-sm text-body">
                                                        ...
                                                        <span class="font-weight-bold">
                                                            {{$add_on_qty}} x {{$addon['price'] ?? 0}} {{\App\CentralLogics\Helpers::currency_symbol()}}
                                                        </span>
                                                    </div>
                                                    @php($add_ons_cost += ($addon['price'] ?? 0) * $add_on_qty)
                                                @endforeach
                                            </div>
        ...
        <div class="row justify-content-between align-items-center">
            <div class="col">
                <p class="font-size-sm mb-0">
                    &copy; {{$businessSettings['restaurant_name'] ?? ''}}. <span
                        class="d-none d-sm-inline-block">{{$businessSettings['footer_text'] ?? ''}}</span>
                </p>
            </div>
        </div>
    </div>
