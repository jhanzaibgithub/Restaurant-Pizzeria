                    ...
                    <div class="card-body">
                        <form action="{{route('branch.table.update',[$table['id']])}}" method="post">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="group_id">{{translate('Select_Group')}} <span class="text-danger">*</span></label>
                                        <select name="group_id" id="group_id" class="custom-select" required>
                                            <option value="" disabled>{{ translate('--select--') }}</option>
                                            @foreach($groups as $group)
                                                <option value="{{$group['id']}}" {{ old('group_id', $table->group_id) == $group['id'] ? 'selected' : '' }}>{{$group['title']}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
